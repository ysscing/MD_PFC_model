function [stb_point, ust_point] = ClassifyStability(stb_point, ust_point, fix_point, r1, param)
%     threshold = 1;
    for i = 1 : length(fix_point(1,:))
        y0 = fix_point(:,i);
%         orz = false;
        f0 = solve_Jacobian(0,y0,r1,param);
        [Jacobian, fail] = numjac(@(t,y_jac) solve_Jacobian(t, y_jac, r1, param), 0, y0, f0, 1e-6*ones(size(y0)));
        if any(fail(:) > 0.001)
