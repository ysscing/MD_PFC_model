function [ time_series] = dynamic( T, sp, rp, g_EE,a)
...
%     stim = stim_signal(T, sp, isDelay);
    ...
    count = 1;
    time_series = zeros(2,round((T.T-1.5)/T.dt));
    r.E = [30;0];
    for t = sq_time
        ...
        I.noise = zeros(9,T.trial);%I.noise - T.dt * I.noise/p.tau.AMPA + randn(9,T.trial) * p.sigma_noise * sqrt(T.dt/p.tau.AMPA);
        
        % * Calculate accumulative stimulus difference
        I.dend.E = I.noise(8:9,:) + I.bg.dend + sp.I_stim;
%         if t > 0
%             count = round(t/T.dt);
%             I.dend.E = I.dend.E + stim(:,:,count); % apply stimulation to dendrite
%         end
        if t > 0 && t < 1
            I.dend.E = I.dend.E + 0.1 * sp.I_stim *[1;-1];
        end
        if t > 2 && t < 2.1
            I.dend.E = I.dend.E + a * sp.I_stim *[-1;1];
        end
%         if t >= 3 && t < 3.1
%             I.dend.E = I.dend.E + 0.1*a * sp.I_stim *[-1;1];
%         end
...
%         if count <= 1000
%             I_soma_dend = 0.001 * count * I_soma_dend;
%         end
