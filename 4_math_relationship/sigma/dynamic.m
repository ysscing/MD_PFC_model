function [ decision_result, accu_stim ] = dynamic( T, sp, rp, g_EE)
    % * * Model dynamic evolution
    
    sq_time = -0.3 : T.dt : T.T;
    [g, p, f, r, s, I] = paramete( g_EE, rp.D1, T.trial);
    inst_stim = zeros(T.T/T.dt, T.trial);
    accu_stim = zeros(T.T/T.dt, T.trial);
    isDelay = 1; %
    stim = stim_signal(T, sp, isDelay);
    win = round(T.win / T.dt);
    ...
    for t = sq_time
        ...
        if t > 0
            ...
            inst_stim(count,:) = I.dend.E(1,:) - I.dend.E(2,:); % instantaneous stimulus difference
            % If t < length of window, accu_stim is incomplete
            if count == 1
                accu_stim(count,:) = inst_stim(count,:);
            elseif count > 1 && count <= win
                accu_stim(count,:) = accu_stim(count-1,:) + inst_stim(count,:);
            elseif count > win
                accu_stim(count,:) = accu_stim(count-1,:) + inst_stim(count,:) - inst_stim(count-win,:);
            end
        end
    ...
    accu_stim = sign(accu_stim); % just need to know positive or negative
